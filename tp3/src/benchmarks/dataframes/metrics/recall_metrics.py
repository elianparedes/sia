...
def recall_metrics(max_epoch, network: NeuralNetwork, step, training_set, expected_set, test_set, test_expected,
    ...
    for epoch in range(step, max_epoch, step):
        w_min, _, _, _ = network.train(training_set, expected_set, test_set, test_expected, batch_amount, epoch,
                                       epsilon)
        training_metrics = network.compute_metric(w_min, training_set, expected_set, Recall.calculate, classifier,
                                                  classes)
        test_metrics = network.compute_metric(w_min, test_set, test_expected, Recall.calculate, classifier, classes)
